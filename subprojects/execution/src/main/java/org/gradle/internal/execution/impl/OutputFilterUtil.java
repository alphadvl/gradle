    ...
    private static class SnapshotFilteringVisitor implements RootTrackingFileSystemSnapshotHierarchyVisitor {
        ...
        public void leaveDirectory(CompleteDirectorySnapshot directorySnapshot, boolean isRoot) {
            ...
            EmptyDirectoryHandlingStrategy emptyDirectoryHandlingStrategy = isOutputDir
                ? INCLUDE_EMPTY_DIRS
                : EXCLUDE_EMPTY_DIRS;
            boolean includedDir = merkleBuilder.leaveDirectory(emptyDirectoryHandlingStrategy, directorySnapshot.getAccessType(), directorySnapshot.getAbsolutePath(), directorySnapshot.getName());
            if (!includedDir) {
                currentRootFiltered = true;
                hasBeenFiltered = true;
            }
